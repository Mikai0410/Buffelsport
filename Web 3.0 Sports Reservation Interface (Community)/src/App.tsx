...
export default function App() {
  ...
  const handleProfileSave = (data: { name: string; email: string; phone: string }) => {
    setUserData({
      ...userData,
      ...data,
    });
  ...
  const handlePaymentComplete = () => {
    ...
    toast.success('Betaling succesvol! Je boeking is bevestigd.', { duration: 3000 });
        ...
        <>
          {isLoggedIn ? (
            <Dashboard 
              ...
              onLogout={handleLogout}
              onDeleteBooking={(id) => setBookings((prev) => prev.filter(b => b.id !== id))}
            />
          ) : (
